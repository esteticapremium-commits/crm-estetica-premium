...
const priorityFirst = (a: SalesTask, b: SalesTask) => Number(Boolean(b.is_priority)) - Number(Boolean(a.is_priority)) || new Date(a.due_at).getTime() - new Date(b.due_at).getTime();

export default function Tasks({ client, pipeline, meName, admin, headerTools, onOpenLead }: { client: Client; pipeline: Pipeline | null; meName: string; admin: boolean; headerTools?: ReactNode; onOpenLead: (lead: Lead) => void }) {
...
function TaskForm({ item, leads, clientId, meName, onClose, onSaved }: { item: SalesTask | null; leads: Lead[]; clientId: string; meName: string; onClose: () => void; onSaved: () => void }) {
  ...
  async function remove() {
    ...
    if (item.lead_id) {
      const { data: nextTask } = await supabase.from("sales_tasks").select("due_at").eq("lead_id", item.lead_id).is("completed_at", null).order("due_at").limit(1).maybeSingle();
      await supabase.from("leads").update({ next_action_date: nextTask?.due_at ? dayOf(nextTask.due_at) : null }).eq("id", item.lead_id);
    }
