...
function TaskCard({ item, onEdit }: { item: PersonalTask; onEdit: (item: PersonalTask) => void }) {
  return <button className={`personal-task-card${item.is_priority ? " priority" : ""}`} onClick={() => onEdit(item)}>{item.is_priority && <em className="task-priority-badge">Prioritaria</em>}<b>{item.title}</b>{item.notes && <small>{item.notes}</small>}<span>Apri per modificare</span></button>;
}
...
function TaskColumn({ column, items, onEdit }: { column: typeof COLUMNS[number]; items: PersonalTask[]; onEdit: (item: PersonalTask) => void }) {
  ...
  const ordered = [...items].sort((a, b) => Number(Boolean(b.is_priority)) - Number(Boolean(a.is_priority)) || a.position - b.position);
  return <section ref={setNodeRef} className={`personal-task-column ${column.id}${isOver ? " drop-hint" : ""}`}><header><div><h2>{column.label}</h2><small>{column.hint}</small></div><span>{items.length}</span></header>{ordered.map((item) => <DraggableTask key={item.id} item={item} onEdit={onEdit} />)}{items.length === 0 && <p className="personal-task-empty">Trascina qui una task</p>}</section>;
}
...
function TaskForm({ item, onClose, onSaved }: { item: PersonalTask | null; onClose: () => void; onSaved: () => void }) {
  const [title, setTitle] = useState(item?.title ?? ""); const [notes, setNotes] = useState(item?.notes ?? ""); const [status, setStatus] = useState<PersonalTaskStatus>(item?.status ?? "backlog"); const [isPriority, setIsPriority] = useState(Boolean(item?.is_priority)); const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null);
  const save = async () => { if (!title.trim()) return setError("Scrivi il titolo della task."); setBusy(true); setError(null); const data = { title: title.trim(), notes: notes.trim() || null, status, is_priority: isPriority }; const result = item ? await supabase.from("personal_tasks").update(data).eq("id", item.id) : await supabase.from("personal_tasks").insert({ ...data, position: 0 }); setBusy(false); if (result.error) setError(result.error.message); else onSaved(); };
